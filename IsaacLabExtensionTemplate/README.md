...
- Throughout the repository, the name `foobar` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of foobar (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e exts/foobar
```
